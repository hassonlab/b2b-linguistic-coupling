...
def verify_args(args, sf):
    ...
    assert args.window * sf == int(args.window *
                                   sf), 'window must be multiple of sf'
...
def decoding(S, E, kfold, sigmask=None, pca=0):
    ...
    for k, (train_index, test_index) in enumerate(kfold.split(S)):
        ...
        for i in range(nlags):
            ...
            corrs2[test_index, i] = utils.correlate(preds, E_test, axis=1).astype(np.float32)
...
def pls(S, E, kfold, sigmask=None, n_components=10):
    ...
    for k, (train_index, test_index) in enumerate(kfold.split(S)):
        ...
        S_latent_train[train_index], E_latent_train[train_index] = model.fit_transform(S_train, E_train)
...
def encoding(sub, speaker, model, tmax, window, jump, k=10, pca=50, alphas=None, reg=0,
             subset=None, band='highgamma', shuffle=False, shift=0,
             slim_results=False, decode=None):
    ...
    if decode is not None:
        sigmask = None
        if decode == 'sig':
            with open('dataset/derivatives/encoding/sigmasks_shuffle-phase_model-gpt2-xl_layer-24_shift.pkl', 'rb') as f:
                sigmasks = pickle.load(f)
            sigmask = sigmasks[(sub, 'prod' if sub == speaker else 'comp', 'gpt2-xl_layer-24_shift')]
        ...
        save_cols = ['word_idx', 'word', 'token', 'top_pred', 'rank', 'true_prob', 'entropy']
        results['df'] = df[save_cols]
        results['args'] = {'model': model, 'pca': pca, 'shift': shift, 'decode': decode, 'tmax': tmax, 'window': window, 'jump': jump, 'shuffle': slim_results}
        return results
    ...
    for train_index, test_index in kfold.split(S):
        ...
        if reg == 1:
            steps.append(SparseGroupLassoCV(l1_regs=alphas, l21_regs=[0], groups=None, solver_params=dict(max_iter=100)))
        elif reg == 2:
            if len(alphas) == 1:
        ...
        if not slim_results:
            ...
            coefs = linmodel.coef_.reshape(-1, nelecs, nlags)  # TODO himalaya (n_features, n_targets)  e.g 50 x n
            intercept = linmodel.intercept_.reshape(nelecs, nlags)  # TODO himalaya (n_features, n_targets)  e.g 50 x n
...
def main(args, subject, speaker):
    ...
    suffix_ext = f'_mode-{mode}_band-{args.band}'
    ...
    outpath = path.copy()
    outpath.update(datatype=dtypename,
    ...
    # Run encoding
    result = encoding(subject, speaker, args.model, args.tmax, args.window, args.jump, alphas=alphas, 
                      reg=args.reg, k=args.nfolds, subset=args.subset, band=args.band,
                      shuffle=args.shuffle, shift=args.shift, pca=ncomp,
                      decode=args.decode, slim_results=args.shuffle is not None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--window', type=float, default=.250)  # s
    parser.add_argument('--jump', type=float, default=2*.03125)  # s
