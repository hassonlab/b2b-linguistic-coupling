...
def add_phonetic_embeddings(df, model, **kwargs):
    """

    Run `nltk.download('cmudict')` once beforehand.
...
UW,vowel,,,,high,back,,"""
    ...
    # Build one-hot coding of phonemes
    phone_set = dfp.phoneme.to_numpy()
    ...
    embeddings = df.word.apply(get_word_phone_emb)
    embeddings = df.word.apply(get_word_phone_features)
...
def add_symbolic_embeddings(df, model, **kwargs):
    # see https://github.com/hassonlab/247-pickling/blob/0shot-paper-3/code/tfsemb_symbolic.py
    import spacy
    ...
    nlp.tokenizer.add_special_case("gotta", [{ORTH: "gotta"}])
    nlp.tokenizer.add_special_case("Gotta", [{ORTH: "Gotta"}])

    # don't -> do 'nt
    # that's' -> that 's
    linguistic_features = []
    for _, group in df.groupby("utterance_id"):
        text = group.utterance.iloc[0]
        text = text.replace("[inaudible]", "")
        text = text.replace("{inaudible}", "")
        text = text.replace("{inaudbile}", "")
        text = text.replace("{Gasps}", "")
        text = text.replace("{LG}", "")
        text = text.replace("-", " ")
        text = text.replace(" cant ", " can't ")  # typos
        ...
        for token in doc:
            if (
                ...
                continue
            tokens.append([token.text, token.tag_, token.dep_, token.is_stop])

        if len(tokens) != len(group):
            print(group.utterance.iloc[0])
            print(text)
            print(len(tokens), len(group))
            print(list(zip(group.word, [t[0] for t in tokens])))
            breakpoint()

        linguistic_features.extend(tokens)

    df2 = pd.DataFrame(
        linguistic_features, columns=["token", "pos", "dep", "stop"], index=df.index
    )
    df = pd.concat([df, df2], axis=1)

    # # get part of speech
    # _, df['upos'] = zip(*pos_tag(df.word, tagset="universal"))
    # df['prev_pos'] = df['upos'].shift(1, fill_value='NOUN')
    # # get stop words
    # stop_words = set(stopwords.words('english'))
    # df['is_stop'] = df.word.str.lower().isin(stop_words)

    # generate embeddings
    le = LabelBinarizer()
    # a = le.fit_transform(df.upos.tolist())
    # b = le.fit_transform(df.prev_pos.tolist())
    # c = le.fit_transform(df.is_stop.tolist())
    a = le.fit_transform(df.pos.tolist())
    b = le.fit_transform(df.dep.tolist())
    c = le.fit_transform(df.stop.tolist())
...
def add_embeddings(df, model, **kwargs):
    ...
    elif "symbolic" in model:
        return add_symbolic_embeddings(df, model, **kwargs)
    elif "phonetic" in model:
        return add_phonetic_embeddings(df, model, **kwargs)
    else:
...
def main(args):
    path = DerivativeBIDSPath(
        subject=args.subject, task=args.task, root=args.root, check=False
    )
...
def copy_to_partners(args):
    ...
    for sub in args.subject:
        ...
        outpath.update(
            subject=f"{sub:02d}",
            root=f"{args.root}/derivatives/embeddings",
            extension=".pkl",
        )
